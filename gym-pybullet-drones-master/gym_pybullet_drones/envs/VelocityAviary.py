...
class VelocityAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        if drone_model in [DroneModel.CF2X, DroneModel.CF2P, DroneModel.CF26]:
            self.ctrl = [DSLPIDControl(drone_model=DroneModel.CF26) for i in range(num_drones)]
    ...
    def _observationSpace(self):
        ...
        if self.nb_prop == 4:
            ...
            obs_lower_bound = np.array([-np.inf, -np.inf, 0.,     -1., -1., -1., -1., -np.pi, -np.pi, -np.pi, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, 0.,           0.,           0.,           0.,           0.,           0.])
            obs_upper_bound = np.array([np.inf, np.inf, np.inf,    1., 1., 1., 1., np.pi, np.pi, np.pi, np.inf, np.inf, np.inf, np.inf,  np.inf,  np.inf,           self.MAX_RPM, self.MAX_RPM, self.MAX_RPM, self.MAX_RPM, self.MAX_RPM, self.MAX_RPM])
    ...
    def _preprocessAction(self,
        ...
        """
        rpm = np.zeros((self.NUM_DRONES, self.nb_prop))
