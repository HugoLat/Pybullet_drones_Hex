...
class DroneModel(Enum):
    ...
    CF26 = "cf26"   # Modified Bitcraze Craziflie 2.0 Hexacopter configuration
...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        print("[INFO] BaseAviary.__init__() loaded parameters from the drone's .urdf:\n[INFO] m {:f}, Nombre of prop {:f} L {:f},\n[INFO] ixx {:f}, iyy {:f}, izz {:f},\n[INFO] kf {:f}, km {:f},\n[INFO] t2w {:f}, max_speed_kmh {:f},\n[INFO] gnd_eff_coeff {:f}, prop_radius {:f},\n[INFO] drag_xy_coeff {:f}, drag_z_coeff {:f},\n[INFO] dw_coeff_1 {:f}, dw_coeff_2 {:f}, dw_coeff_3 {:f}".format(
            self.M, self.nb_prop, self.L, self.J[0,0], self.J[1,1], self.J[2,2], self.KF, self.KM, self.THRUST2WEIGHT_RATIO, self.MAX_SPEED_KMH, self.GND_EFF_COEFF, self.PROP_RADIUS, self.DRAG_COEFF[0], self.DRAG_COEFF[2], self.DW_COEFF_1, self.DW_COEFF_2, self.DW_COEFF_3))
        #### Compute constants #####################################
        ...
        elif self.DRONE_MODEL in [DroneModel.CF2P, DroneModel.HB, DroneModel.CF26]:
        ...
        if self.DYNAMICS_ATTR:
            if self.DRONE_MODEL == DroneModel.CF2X:
                ...
                self.A = np.array([ [1, 1, 1, 1, 1, 1], [.5, 1, .5, -.5, -1, -.5], [-np.sqrt(3)/2, 0, np.sqrt(3)/2, -np.sqrt(3)/2, 0, -np.sqrt(3)/2], [-1, 1, -1, 1, -1, 1] ])
    ...
    def step(self,
        ...
        #### Save, preprocess, and clip the action to the max. RPM #
            ...
            clipped_action = np.reshape(self._preprocessAction(action), (self.NUM_DRONES, self.nb_prop))
    ...
    def _housekeeping(self):
        ...
        self.last_action = -1*np.ones((self.NUM_DRONES, self.nb_prop))
        self.last_clipped_action = np.zeros((self.NUM_DRONES, self.nb_prop))
        self.gui_input = np.zeros(self.nb_prop)
    ...
    def _getDroneStateVector(self,
        ...
        return state.reshape(int(16+self.nb_prop),)
    ...
    def _physics(self,
        ...
        z_torque = 0
        for i in range(int(self.nb_prop)):
            z_torque = (-1)**(i+1)*torques[i] + z_torque
        for i in range(int(self.nb_prop)):
    ...
    def _groundEffect(self,
        ...
        link_states = np.array(p.getLinkStates(self.DRONE_IDS[nth_drone],
                                               linkIndices=range(self.nb_prop+1),
    ...
    def _dynamics(self,
        ...
        z_torque = 0
        for i in range(int(self.nb_prop)):
            z_torque = (-1)**(i+1)*z_torques[i] + z_torque
        if self.DRONE_MODEL==DroneModel.CF2X:
            ...
            x_torque = (forces[0] + forces[1] - forces[3] - forces[4]) * (self.L/np.sqrt(2))
            y_torque = (- forces[0] + forces[1] + forces[2] + forces[3] -forces[4] -forces[5]) * (self.L/np.sqrt(2))
    ...
    def _saveLastAction(self,
        ...
        if isinstance(action, collections.abc.Mapping):
            for k, v in action.items(): 
                res_v = np.resize(v, (1, self.nb_prop)) # Resize, possibly with repetition, to cope with different action spaces in RL subclasses
                self.last_action[int(k), :] = res_v
        else: 
            res_action = np.resize(action, (1, self.nb_prop)) # Resize, possibly with repetition, to cope with different action spaces in RL subclasses
            self.last_action = np.reshape(res_action, (self.NUM_DRONES, self.nb_prop))
    ...
    def _parseURDFParameters(self):
        ...
        return M, nb_prop, L, THRUST2WEIGHT_RATIO, J, J_INV, KF, KM, COLLISION_H, COLLISION_R, COLLISION_Z_OFFSET, MAX_SPEED_KMH, \
