...
class Logger(object):
    ...
    def __init__(self,
        ...
        self.states = np.zeros((num_drones, 12+self.nb_prop, duration_sec*self.LOGGING_FREQ_HZ)) #### 16 states: pos_x,
                                                                                                  # pos_y,
                                                                                                  # pos_z,
                                                                                                  # vel_x,
                                                                                                  # vel_y,
    ...
    def log(self,
        ...
        """
        if drone < 0 or drone >= self.NUM_DRONES or timestamp < 0 or len(state) != 16+self.nb_prop or len(control) != 12:
        ...
        if current_counter >= self.timestamps.shape[1]:
            ...
            self.states = np.concatenate((self.states, np.zeros((self.NUM_DRONES, 12+self.nb_prop, 1))), axis=2)
        ...
        self.states[drone, :, current_counter] = np.hstack([state[0:3], state[10:13], state[7:10], state[13:(16+self.nb_prop)]])
    ...
    def plot(self, pwm=False):
        ...
        for j in range(self.NUM_DRONES):
            for i in range(12,(12+self.nb_prop)):
                if pwm and j > 0:
                    self.states[j, i, :] = (self.states[j, i, :] - 4070.3) / 0.2685
        t = np.arange(0, self.timestamps.shape[1] / self.LOGGING_FREQ_HZ, 1 / self.LOGGING_FREQ_HZ)
        fig, axs = plt.subplots(self.nb_prop, 1)
        col = 0
        for i in range(self.nb_prop):
            for j in range(self.NUM_DRONES):
                axs[i].set_xlabel('time')
                axs[i].set_ylabel('RPM'+str(i))
                axs[i].plot(t, self.states[0, 12+i, :], label="moteur "+str(i)+ " drone "+str(j))
        for i in range(self.nb_prop):
            axs[i].grid(True)
